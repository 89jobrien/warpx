...
pub enum Harness {
    ...
    #[value(name = "gemini")]
    Gemini,
    /// Delegate to the `braid-agent` CLI (personal fork harness).
    #[value(name = "joe", alias = "braid")]
    Joe,
...
impl Harness {
    ...
    pub fn parse_local_child_harness(value: &str) -> Option<Self> {
        match Self::parse_orchestration_harness(value) {
            ...
            Some(Self::Oz) | Some(Self::Gemini) | Some(Self::Joe) | Some(Self::Unknown) | None => {
                None
            }
        }
    }
